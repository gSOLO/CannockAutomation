 ...
 */
WemoControllee.prototype.getBinaryState = function () {
    var self = this;
    var args = { };
    this.eventService.callAction(ACTION_GetBinaryState, args, function (err, buf) {
        if (err) {
            console.log("got err when performing action: " + err + " => " + buf);
        }
        else {
...
/**
 * A UPnP WeMo Lightswitch.  Includes socket switch.
 */
var WemoLightswitch = function (device) {
    EventEmitter.call(this);
    ...
    if (this.eventService) {
        this.eventService.on("stateChange", function (value) {
            if (TRACE && DETAIL) {
                console.log("wemo light switch state change: " + JSON.stringify(value));
            }
            if (value["BinaryState"]) {
                self.emit("BinaryState", value.BinaryState);
            }
...
util.inherits(WemoLightswitch, EventEmitter);

WemoLightswitch.deviceType = "urn:Belkin:device:lightswitch:1";

/**
 * send state change to device
 */
WemoLightswitch.prototype.setBinaryState = function (value) {
    var self = this;
    var v = value ? 1 : 0;
    var args = { BinaryState: v };
    this.eventService.callAction(ACTION_SetBinaryState, args, function (err, buf) {
 ...
 * get state from device
 */
WemoLightswitch.prototype.getBinaryState = function () {
    var self = this;
    var args = {};
    this.eventService.callAction(ACTION_GetBinaryState, args, function (err, buf) {
        if (err) {
...
exports.devices = [WemoControllee, WemoLightswitch, WemoSensor];
exports.WemoControllee = WemoControllee;
exports.WemoLightswitch = WemoLightswitch;
...
const ACTION_GetBinaryState = "GetBinaryState";
